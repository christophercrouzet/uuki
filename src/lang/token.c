...
// Token Scanning: Integer Literals
// ---------------------------------------------------------------- //   O-(''Q)

static enum w_status
wp_token_scan_binary_int_literal(
    struct w_tokenizer *tokenizer,
    enum w_token_id *id
)
{
    ...
    switch (*tokenizer->it)
    {
        case WP_TOKEN_CASE_BINARY_DIGITS:
        {
            ...
            switch (*tokenizer->it)
            {
                ...
                {
                    *id = W_TOKEN_ID_LITERAL_INT_BINARY;
                    return W_SUCCESS;
                }
                default:
                {
...
static enum w_status
wp_token_scan_octal_int_literal(
    struct w_tokenizer *tokenizer,
    enum w_token_id *id
)
{
    ...
    switch (*tokenizer->it)
    {
        case WP_TOKEN_CASE_OCTAL_DIGITS:
        {
            ...
            switch (*tokenizer->it)
            {
                case WP_TOKEN_CASE_VALID_FOLLOW_UP_TO_NUMBER_LITERAL:
                {
                    *id = W_TOKEN_ID_LITERAL_INT_OCTAL;
                    return W_SUCCESS;
                }
                default:
                {
...
// Token Scanning: Floating Literals
// ---------------------------------------------------------------- //   O-(''Q)

static enum w_status
wp_token_scan_flt_exponent_literal(
    struct w_tokenizer *tokenizer,
    enum w_token_id *id,
...
)
{
    ...
    switch (*tokenizer->it)
    {
        case WP_TOKEN_CASE_DECIMAL_DIGITS:
        {
            ...
            switch (*tokenizer->it)
            {
                ...
                default:
                {
                    return wp_token_add_error(
                        tokenizer,
                        WP_TOKEN_GET_ERROR_MSG_INVALID_CHAR("floating exponent")
                    );
                }
            }
        }
        case WP_TOKEN_CASE_VALID_FOLLOW_UP_TO_NUMBER_LITERAL:
        {
            return wp_token_add_error(
                tokenizer,
                WP_TOKEN_GET_ERROR_MSG_UNTERMINATED("floating exponent")
            );
        }
        default:
        {
...
static enum w_status
wp_token_scan_decimal_flt_literal(
    struct w_tokenizer *tokenizer,
    enum w_token_id *id
)
{
    ...
    switch (*tokenizer->it)
    {
        ...
        {
            wp_token_advance_iter(tokenizer);
            return wp_token_scan_flt_exponent_literal(
                tokenizer, id, W_TOKEN_ID_LITERAL_FLT_DECIMAL
            );
        }
        case WP_TOKEN_CASE_VALID_FOLLOW_UP_TO_NUMBER_LITERAL:
        {
            *id = W_TOKEN_ID_LITERAL_FLT_DECIMAL;
            return W_SUCCESS;
        }
        default:
        {
            return wp_token_add_error(
                tokenizer,
                WP_TOKEN_GET_ERROR_MSG_INVALID_CHAR("decimal floating literal")
            );
        }
    }
}
...
static enum w_status
wp_token_scan_hexadecimal_flt_literal(
    struct w_tokenizer *tokenizer,
    enum w_token_id *id
)
{
    ...
    switch (*tokenizer->it)
    {
        case 'p':
        {
            wp_token_advance_iter(tokenizer);
            return wp_token_scan_flt_exponent_literal(
                tokenizer, id, W_TOKEN_ID_LITERAL_FLT_HEXADECIMAL
            );
        }
        case WP_TOKEN_CASE_VALID_FOLLOW_UP_TO_NUMBER_LITERAL:
        {
            *id = W_TOKEN_ID_LITERAL_FLT_HEXADECIMAL;
            return W_SUCCESS;
        }
        default:
        {
...
static enum w_status
wp_token_scan_decimal_number_literal(
    struct w_tokenizer *tokenizer,
    enum w_token_id *id
)
{
    ...
    switch (*tokenizer->it)
    {
        ...
        {
            wp_token_advance_iter(tokenizer);
            return wp_token_scan_decimal_flt_literal(
                tokenizer, id
            );
        }
        case 'e':
        ...
        case WP_TOKEN_CASE_VALID_FOLLOW_UP_TO_NUMBER_LITERAL:
        {
            *id = W_TOKEN_ID_LITERAL_INT_DECIMAL;
            return W_SUCCESS;
        }
        default:
        {
...
static enum w_status
wp_token_scan_hexadecimal_number_literal(
    struct w_tokenizer *tokenizer,
    enum w_token_id *id
)
{
    ...
    switch (*tokenizer->it)
    {
        case WP_TOKEN_CASE_HEXADECIMAL_DIGITS:
        {
            ...
            switch (*tokenizer->it)
            {
                ...
                {
                    wp_token_advance_iter(tokenizer);
                    return wp_token_scan_hexadecimal_flt_literal(
                        tokenizer, id
                    );
                }
                case 'p':
                ...
                case WP_TOKEN_CASE_VALID_FOLLOW_UP_TO_NUMBER_LITERAL:
                {
                    *id = W_TOKEN_ID_LITERAL_INT_HEXADECIMAL;
                    return W_SUCCESS;
                }
                default:
                {
        ...
        {
            wp_token_advance_iter(tokenizer);
            return wp_token_scan_hexadecimal_flt_literal(tokenizer, id);
        }
        case 'p':
        {
            return wp_token_add_error(
        ...
        case WP_TOKEN_CASE_VALID_FOLLOW_UP_TO_NUMBER_LITERAL:
        {
            return wp_token_add_error(
                tokenizer,
                WP_TOKEN_GET_ERROR_MSG_UNTERMINATED("hexadecimal number literal")
            );
        }
        default:
        {
        ...
        switch (*tokenizer->it)
        {
            ...
            case '0':
            {
                ...
                switch (*tokenizer->it)
                {
                    ...
                    {
                        wp_token_advance_iter(tokenizer);
                        status = wp_token_scan_binary_int_literal(
                            tokenizer, &id
                        );
                        goto exit;
                    }
                    ...
                    {
                        wp_token_advance_iter(tokenizer);
                        status = wp_token_scan_octal_int_literal(
                            tokenizer, &id
                        );
                        goto exit;
                    }
                    ...
                    {
                        wp_token_advance_iter(tokenizer);
                        status = wp_token_scan_hexadecimal_number_literal(
                            tokenizer, &id
                        );
                        goto exit;
                    }
                    ...
                    {
                        wp_token_advance_iter(tokenizer);
                        status = wp_token_scan_decimal_number_literal(
                            tokenizer, &id
                        );
                        goto exit;
                    }
                    ...
                    {
                        wp_token_advance_iter(tokenizer);
                        status = wp_token_scan_decimal_flt_literal(
                            tokenizer, &id
                        );
                        goto exit;
                    }
                    ...
                    {
                        wp_token_advance_iter(tokenizer);
                        status = wp_token_scan_flt_exponent_literal(
                            tokenizer, &id, W_TOKEN_ID_LITERAL_FLT_DECIMAL
                        );
                        goto exit;
                    }
            ...
            {
                wp_token_advance_iter(tokenizer);
                status = wp_token_scan_decimal_number_literal(tokenizer, &id);
                goto exit;
            }
            case '.':
            {
